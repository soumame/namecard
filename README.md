...
Maker Faire Tokyo 2026のブース「そうまめの部屋」向けに製作しました。
...
- [`client/ios/`](client/ios/) — iOSアプリ（TestFlightベータ配布）
- [`iOS_development.md`](iOS_development.md) — iOS版の実装構成と今後の検証計画
...
このアプリが要求するAndroid権限はNFCのみです。リリースにはAPKと一緒にSHA-256チェックサムを掲載しています。[Releases](https://github.com/soumame/namecard/releases)の`android-main`は最新版で上書きされます。ソースの変更履歴は[コミット一覧](https://github.com/soumame/namecard/commits/main/)で確認できます。

うまく書き込めない場合は、スマートフォンのNFCアンテナ位置を確認し、ケースを外してからもう一度お試しください。書き込み中は名刺を動かしたり、ほかのアプリへ切り替えたりしないでください。

## iOSアプリ

iOS 17以降のNFC対応iPhone向けに、MameCardをTestFlightでベータ配布しています。

**[TestFlight事前登録フォームに回答する](https://forms.gle/RqQPmfoJ9aMMtRm66)**

フォームへの回答後に、TestFlightの参加リンクをお送りします。届いたリンクから案内に沿って
インストールしてください。名刺への書き込みには対応する名刺基板が必要です。

iPhone XR／iOS 18で白黒画像・URL設定・クリーニング・振動案内の動作報告を受けています。
ほかの機種での確認と給電・時間の計測を進めます。試す内容と報告方法は
...
- TestFlightでベータ版を利用できます。[事前登録フォーム](https://forms.gle/RqQPmfoJ9aMMtRm66)への回答後に、TestFlightの参加リンクをお送りします。実機検証後にApp Store公開を目指します。
