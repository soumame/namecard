...
Newの`URL`からHTTP(S)のURLを書き込むか、`URLをクリア`を選び、名刺へタッチします。
クリアにはURL入力は不要です。空欄を通常の書き込みとして確定しても削除されません。
処理中は名刺を固定し、読み返し確認が完了するまで待ってください。

クリアはURLレコードを1件の空のNDEFレコード（`TNF_EMPTY`）へ置き換えます。
タッチ時に開くURLがなくなりますが、NFCタグ自体の検出、画像更新、URLの再設定は可能です。
画像・FW・ロック設定は変更しません。既存のURL書き込み対応FWをそのまま利用します。
中断時は、同じ名刺を離して再タッチすると保持したクリア操作を再試行します。
Androidでは既存のURL設定と同様、再試行情報はアプリ起動中のみ保持します。

方式の根拠: [Android NdefRecord](https://developer.android.com/reference/android/nfc/NdefRecord#TNF_EMPTY)。
実機ではURL設定→クリア→通常タッチでURLが開かない→画像更新→URL再設定を確認します。
クリア済み名刺の再クリアと、通信中断後の再タッチも確認します。

## QRコード

Newの編集ツールで`URL`の隣にある`QRコード`を押し、モーダルへURLを入力して
`QRコードを生成`を選びます。プレビューを確認して`名刺に追加`を押すと、画像レイヤーとして
キャンバスへ追加できます。移動・拡縮・回転・重なり順・Undo／Redoは既存の画像と同じです。
Library保存・BIN書き出し・NFC画像書き込みにも含まれます。

URLは既存のURL設定と同じHTTP(S)の検証・補完を行い、端末内でZXing Core 3.5.4を使って生成します。
生成だけで名刺のNFC URLは変わりません。入力を変更すると古いプレビューを消し、再生成が必要になります。
QRの周囲に4セルの白い余白を確保し、初期配置は最大120×120ピクセル、1セル2ピクセル以上の
整数倍率とします。これより細かくなる長いURLはプレビューのみ可能で、名刺へ追加するには短いURLが必要です。
配置後に小さく縮小したり、他の要素で余白を隠した場合の読み取りは保証しません。

生成処理: [ZXing QRCodeWriter](https://github.com/zxing/zxing/blob/zxing-3.5.4/core/src/main/java/com/google/zxing/qrcode/QRCodeWriter.java)
（Apache License 2.0）。
...
古い接続と転送処理の終了を待ち、Reader Modeを停止して300ms後に再初期化する。
以前の`ignore()`による取り除き待ちは使用せず、古い探索から遅れて届く検出通知を破棄する。
再初期化の間も通常のNFC通知をこのアプリで受け、名刺のURLから別アプリが開くのを防ぐ。
ホーム画面へ戻らずに復旧するための修正で、連続書き込みの追加修正後に動作成功とのユーザー報告あり。
今回確認した端末がPixel 9 Pro／POCO X3 NFCのどちらか、両方かと、正確な試行回数は未記録。
ファイルを選び直さず、そのまま再タッチすると同じDATAから
...
アプリを背景へ移した際は転送をキャンセルし、画像用の`NfcV`だけでなくURL用の
`Ndef`／`NdefFormatable`も閉じる。接続開始とキャンセルが重なった場合にも、古い処理と
接続の解放が完了するまで新しい転送を開始しない。

正常終了後に次の画像を書き込む場合も、書き込みボタンを押した時点で探索を停止・再開する。
画像・URL・STATUS・内蔵パターンの各開始経路で同じ再初期化を使い、画像とURLを切り替えた際は
新しいNDEF確認設定でタグを取得する。更新中のquiet時間を守るため、完了した瞬間の探索リセットは行わない。
以前は画像の開始時に探索を更新せず、前回または操作選択前に検出したタグが残る経路があった。
...
実機では同じ名刺へ画像A→画像B→画像Aを続けて書き、URL設定→画像更新も確認する。
ホームへ戻らず新しい画像まで完了すること、書き込み画面で名刺のURLが開かないことを確認する。
再現時は、書き込み開始直後の`新しい操作のためNFC探索を再初期化`、`NFC探索開始`、
`通常NFC通知をアプリで受信`、`NFC-V検出 UID`の有無と端末名・OSを記録する。
修正後、ユーザーから「うまく動きました」との報告を受けた（2026-09-08）。
連続書き込みの改善確認として記録し、両機種での成功や特定回数の連続成功までは推定しない。
