...
- **New:** 画像・日本語テキスト、テキスト追加時のフォント・太字／斜体／下線、移動／サイズ／回転、重なり、Undo／Redo、グリッド・位置スナップ、紙面表示の変換・表示回転スナップ。
...
- **URL:** HTTP(S)のNDEF書込・URLクリア・読み返し・Mailbox復旧。既知の未フォーマットST25DV04Kにも対応します。
- **QRコード:** URLの隣のボタンから入力フォームを開き、生成プレビューを確認して名刺へ画像として追加できます。
...
Newの`URL`の隣の`QRコード`を押し、URL入力→`QRコードを生成`→プレビュー→`名刺に追加`の順で操作します。
生成画像は通常の画像レイヤーと同様に移動・拡縮・回転・重なり順変更・Undo／Redoでき、
Library・BIN・NFC画像書き込みにも含まれます。QR生成で名刺のNFC URLは変更されません。

HTTP(S)の検証・補完は既存のURL設定と共通です。生成は端末内の
[Core Image QRコード生成器](https://developer.apple.com/documentation/coreimage/ciqrcodegenerator)を利用します。
URL変更時は古いプレビューを消去し、再生成が必要になります。
白い余白を4セル以上確保し、初期配置は最大120×120ピクセル、1セル2ピクセル以上の整数倍率です。
これより細かい長いURLはプレビューのみ可能で、名刺へ追加するには短いURLが必要です。
実物では配置・縮小・回転・余白の重なり・電子ペーパーの状態によって読みやすさが変わるため、書き込み後も読み取れるか確認してください。

## テキストの書式と表示回転スナップ

Newの`テキスト`で文字を入力し、`B`（太字）・`I`（斜体）・`U`（下線）と`フォント`を設定して追加します。
初期値は従来どおりシステムフォント・太字です。フォントは検索でき、入力フォームのプレビュー、
紙面・完成画像・BINで共通の書式を使います。書式は拡縮・回転・Undo／Redoでも保持されます。
フォント一覧は公開APIの[UIFont.familyNames](https://developer.apple.com/documentation/uikit/uifont/familynames)と
`fontNames(forFamilyName:)`から取得するため、OSと端末により異なります。
太字の書体がない場合は、選んだフォントを保ったまま線幅を補います。
斜体は日本語の代替フォントにも反映できるよう、文字全体を傾けます。
そのフォントにない文字はOSの代替フォントで表示されます。

ツールバーの`表示回転スナップ`は、灰色の余白から操作する紙面表示の回転に適用されます。
初期OFFで、ONにすると15°ごとの角度に4°以内で吸着します。既存の要素の位置スナップとは独立しています。
ゆっくり回しても吸着から抜けられ、表示の拡縮・移動と同時に使えます。
表示回転・スナップ設定は完成画像やBINには含まれません。

`要素回転スナップ`は画像・テキストの回転に適用される独立トグルです。初期OFFで、
ONにすると紙面に対する要素の角度を15°間隔・±4°以内で吸着させます。
位置スナップ・表示回転スナップとは別に切り替えられ、移動・拡縮と同時に使用できます。
要素の回転は完成画像・BINにも反映され、1回のジェスチャーをUndo／Redoできます。
...
画像転送の起動・COMMIT後は1.5秒、EXECUTE後はFWのquiet指定以上、RFコマンドを止めます。
...
Newの`URL`画面で`URLをクリア`を選び、名刺へタッチすると、URLを空のNDEFレコード
（`TNF_EMPTY`、`D0 00 00`）へ置き換えます。URL入力は不要です。空欄の通常書き込みは
従来どおり拒否します。クリア後は通常タッチで開くURLがなくなり、画像更新とURLの再設定は可能です。
NFCタグ自体の検出やロック設定は変更しません。既存のURL書き込み対応FWをそのまま利用します。
背景読取りはURIレコードを対象とします（[Appleの仕様](https://developer.apple.com/documentation/corenfc/adding-support-for-background-tag-reading)）。

URLの設定・クリアはPREPARE応答後だけMailboxを停止します。URL復旧記録をApplication Supportへ原子的に保存し、
未フォーマット品はCC／長さ0／本体／最終長の順に更新します。
中断後の同じUIDで書き込み予定と一致する場合だけ復旧し、未知データや保護領域は初期化しません。
静的MB_MODE、EH_MODE、パスワード、FW、基板は変更しません。

URL処理は最初に4秒RFコマンドを止め、その後は最大8秒、VCC_ONとMailboxの起動を確認します。
通常の上書きでは全メモリの事前読出しを省き、読み返しはCC／TLVを解析してNDEF末尾までに限定します。
未フォーマット品と初期化途中の復旧では、従来どおり512 bytes全体と保護状態を検証します。
副作用のないURL読出しの一時エラーは100／200ms待って最大3回試します。書込・ACK読出しはその場で再送しません。
接続喪失時は同じUIDを最大3回再検出し、元の60秒期限に14秒を超える余裕がある場合だけ自動復旧します。
再接続時に通常のNDEF領域が既に目的の内容なら再書込を省き、読み返しとMailbox再開を確認して完了します。
URLの処理段階、読出しブロック、一時エラーの再試行、再検出はSettingsの通信ログで確認できます。

クリア中断時も操作の種類とUIDを復旧記録へ保存します。アプリ再起動後はSettingsの
`URLクリアを再開`から同じ名刺で再開できます。未完了のURL設定をクリア操作で置き換えることはできません。
旧バージョンのURL設定の復旧記録も引き続き再開できます。
