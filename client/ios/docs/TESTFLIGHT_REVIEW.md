...
以下は2026-09-09の初回提出時点の記録と原稿です。審査状態・テスター数・通知設定は当時の値です。
現在は[事前登録フォーム](https://forms.gle/RqQPmfoJ9aMMtRm66)への回答後にTestFlightの参加リンクを送っています。
参加方法は[ベータ試験ガイド](BETA_TESTING.md)を参照してください。
...
提出時のApp Store Connectでは`0.1.0 (1)`は「審査待ち」でした。
その時点では外部テスターは0人、テスターへの自動通知はOFFで、招待リンクは未作成でした。
...
2. 承認済みビルドが外部テストで配布可能か確認する。招待リンクの有効性と通知設定も確認する。
3. iPhone・iOS 17以降に合わせた募集条件と、専用基板が必要なことを案内する。
4. READMEには事前登録フォームを掲載し、回答者へ有効なTestFlightの参加リンクを送る。
