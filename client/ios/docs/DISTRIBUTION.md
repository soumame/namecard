...
現在は[事前登録フォーム](https://forms.gle/RqQPmfoJ9aMMtRm66)への回答後に、
TestFlightの参加リンクを送る運用です。利用者向けの手順は[ベータ試験ガイド](BETA_TESTING.md)を参照してください。

初回配布の履歴として、2026-09-08にBeta 0.1.0（1）をApp Store Connectへアップロードし、
内部グループ「開発者テスト」へ追加しました。2026-09-09には外部グループ「名刺ユーザーテスト」へ
同ビルドを追加し、ベータ審査へ提出しました。その時点の登録内容は[審査登録資料](TESTFLIGHT_REVIEW.md)に記録しています。
...
リポジトリのVersion／Build設定は`0.1.0（1）`です。次回アップロード前にApp Store Connectの
登録済み番号を確認し、未使用のBuild番号へ増やしてください。
...
7. 購入者など外部テスター向けには、テスト情報・連絡先・専用基板が必要なこと・操作方法を登録し、
   外部テストの審査を受ける。配布可能なビルドと有効な参加リンクを確認し、事前登録フォームの回答者へ案内する。
   リポジトリREADMEには事前登録フォームを掲載する。
